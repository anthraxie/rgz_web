...
const Account = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [userResponse, videosResponse] = await Promise.all([
          axios.get('http://37.194.42.234:8000/api/auth/me/', {
            headers: {
              Authorization: `Bearer ${accessToken}` // Используем полученный токен
            }
          }),
          axios.get('http://37.194.42.234:8000/api/videos/user/', {
            headers: {
              Authorization: `Bearer ${accessToken}` // Используем полученный токен
            }
          })
  ...
  const handleDeleteVideo = async (videoId) => {
    ...
    try {
      await axios.delete(`http://37.194.42.234:8000/api/videos/${videoId}/`, {
        headers: {
          Authorization: `Bearer ${accessToken}` // Используем полученный токен
        }
      });
  ...
  return (
    ...
    <div className={styles.container} style={{
      ...
      <div className={styles.profileSection} style={{
        backgroundColor: 'rgba(255, 255, 255, 0.8)', // Полупрозрачный белый фон для секций
        padding: '20px',
        borderRadius: '8px',
