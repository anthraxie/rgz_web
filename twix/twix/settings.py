"""
Django settings for YadroTube project.
...
SECRET_KEY = 'django-insecure-d9w@^=3262t=))an&yxqlui23^fnzl$4*#)q@lkuabz=12b3tm'
...
ALLOWED_HOSTS = ['*', 'localhost', '127.0.0.1', '37.194.42.234'] # Добавь свой внешний IP/домен сюда
...
# WSGI Application
WSGI_APPLICATION = 'twix.wsgi.application'
...
CORS_ALLOWED_ORIGINS = [
    "http://localhost:3000",         # Стандартный локальный источник фронтенда
    "http://127.0.0.1:3000",       # Альтернативный локальный источник фронтенда
    f"http://{ALLOWED_HOSTS[3]}:3000",  # Добавляем внешний IP с портом фронтенда (если 37.194.42.234 это ALLOWED_HOSTS[3])
    f"http://{ALLOWED_HOSTS[3]}",       # Добавляем внешний IP без порта (если фронтенд отдается на стандартном 80)
    "http://localhost:80",         # Стандартный локальный источник фронтенда
    "http://127.0.0.1:80",       # Альтернативный локальный источник фронтенда
    f"http://{ALLOWED_HOSTS[3]}:80",  # Добавляем внешний IP с портом фронтенда (если 37.194.42.234 это ALLOWED_HOSTS[3])
    f"http://{ALLOWED_HOSTS[3]}",
    # Если ты используешь домен warpy.ru, добавь и его:
    # "http://warpy.ru:3000",
    # "http://warpy.ru",
    # Добавь сюда любые другие точные источники, с которых будет приходить фронтенд
]
